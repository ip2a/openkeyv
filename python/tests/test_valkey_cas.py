"""Valkey store atomic CAS/revision Python tests.

Requires a real Valkey service. Set ``OPENKEYV_VALKEY_URL`` (default
``redis://127.0.0.1:16380``) to enable these integration tests.
"""
...
from openkeyv import ValkeyStore

_VALKEY_URL = os.environ.get("OPENKEYV_VALKEY_URL", "redis://127.0.0.1:16380")
_skip_if_no_valkey = pytest.mark.skipif(
    not os.environ.get("OPENKEYV_VALKEY_URL"),
    reason="requires OPENKEYV_VALKEY_URL",
)


@_skip_if_no_valkey
async def test_valkey_get_with_revision_missing() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    assert await store.get_with_revision("missing") is None


@_skip_if_no_valkey
async def test_valkey_cas_create_if_absent_success() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_cas_create_if_absent_existing_conflict() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v")
...
@_skip_if_no_valkey
async def test_valkey_cas_exact_revision_update() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", 1)
    observed = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_cas_stale_revision_conflict() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", 1)
...
@_skip_if_no_valkey
async def test_valkey_cas_same_value_changes_revision() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_cas_new_ttl_replaces_old_ttl() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_cas_conflict_does_not_refresh_ttl() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v", ttl=100.0)
    observed = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_cas_expired_treated_as_absent() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_compare_and_delete_success() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v")
    observed = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_compare_and_delete_stale_conflict() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v")
    first = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_compare_and_delete_missing_conflict() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_revision_equality_and_hash() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v")
    observed = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_revision_not_orderable() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
    await store.put("k", "v")
    observed = await store.get_with_revision("k")
...
@_skip_if_no_valkey
async def test_valkey_concurrent_cas_exactly_one_success() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_concurrent_create_if_absent_exactly_one_success() -> None:
    store = ValkeyStore(_VALKEY_URL)
    await store.destroy()
...
@_skip_if_no_valkey
async def test_valkey_revision_is_not_constructible() -> None:
