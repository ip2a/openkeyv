...
#[async_trait]
impl<T> AsyncCompareAndSwap for PrefixKeysWrapper<T>
where
    T: AsyncKeyValue + AsyncCompareAndSwap + Send + Sync,
{
    async fn get_with_revision(
        ...
        self.inner
            .get_with_revision(&self.prefixed(key), collection)
            .await
    }
    ...
    async fn compare_and_swap(
        ...
        self.inner
            .compare_and_swap(&self.prefixed(key), expected, value, collection, ttl)
            .await
    }
    ...
    async fn compare_and_delete(
        ...
        self.inner
            .compare_and_delete(&self.prefixed(key), expected, collection)
            .await
    }
}
...
mod tests {
    ...
    fn wrapper_preserves_compare_and_swap_capability() {
        assert_capabilities::<PrefixKeysWrapper<MemoryStore>>();
    }
